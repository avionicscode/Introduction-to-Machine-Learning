...
class SoftmaxClassifier:
        ...
        * loss of one epoch = Mean of minibatch losses
        ...
        # ========================= EDIT HERE ========================
            ...
            for j in range( int(num_data/batch_size)):
                ...
                loss = self.softmax_loss(prob, y_batch)
                wd = self.compute_grad(x_batch, self.W, prob, y_batch)
                batch_losses.append(loss)
                self.W = optimizer.update(self.W, wd, lr)
        # ============================================================
    ...
    def softmax_loss(self, prob, label):
        ...
        log_prob = -np.log(prob[range(batch_size), label])
    ...
    def compute_grad(self, x, weight, prob, label):
        ...
        batch_size = label.shape[0]
        dProb = prob.copy()
        dProb[range(batch_size), label] -= 1
        xT = np.transpose(x)
        grad_weight = np.dot(xT, dProb) / batch_size
        # ============================================================
        return grad_weight
