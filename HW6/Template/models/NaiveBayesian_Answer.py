                ...
                for attr, cnt in zip(attribute, count):
                  # to avoid key Error in predict fuction.
                    if type(attr) == np.float64 or type(attr) == np.float32:
                        attr = str(int(attr))
                    value = attr
                    tmp_likeli = (cnt+1)/(n+len(attribute))
                    likelihood['%s = %s' %(feature_name, value)] = round(tmp_likeli, 6)
            else:
                avg = sum(column_data.values)/float(len(column_data.values))
                variance = sum([pow(x-avg,2) for x in column_data.values])/float(len(column_data.values)-1)
                likelihood['%s_mean' %(feature_name)] = avg
                likelihood['%s_std' %(feature_name)] = np.sqrt(variance) 
        # ================================================
        return
        ...
        for idx, f in enumerate(feature):
            ...
            if idx in self.Category_feature_idx:
              # to avoid key Error in predict fuction.
                ...
                posterior *= Gaussian_prob(val, mean, std)
