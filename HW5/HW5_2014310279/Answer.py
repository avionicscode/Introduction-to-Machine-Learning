...
def convolution2d(x, kernel, stride):
    ...
    for  i in range(conv_height):
        for j in range(conv_width):
            tmp_x = x[i*stride:i*stride+kernel_size, j*stride:j*stride+kernel_size]
...
class ConvolutionLayer:
    ...
    def convolution(self, x, kernel, bias=None, stride=1, pad=0):
        ...
        [Output]
        conv_out : convolution result
        - Shape : (Batch size, Out_Channel, Conv_Height, Conv_Width)
        - Conv_Height & Conv_Width can be calculated using 'Height', 'Width', 'Kernel size', 'Stride'
        """
        batch_size, in_channel, _, _ = x.shape
        if pad > 0:
...
class MaxPoolingLayer:
    ...
    def forward(self, x):
        ...
        - Shape : (Batch_size, Out_channel, Pool_Height, Pool_Width)
            ...
            for c in range(channel):
                for h in range(pool_height):
                    for w in range(pool_width):
                        tmp_mask = self.mask[n, c, h*std : h*std + self.kernel_size, w*std : w*std + self.kernel_size]
                        tmp_x = x[n, c, h*std : h*std + self.kernel_size, w*std : w*std + self.kernel_size]
                        max_pool[n, c, h, w] = np.max(tmp_x)
                        tmp_mask = np.where(tmp_x == np.max(tmp_x), 1, 0)
                        self.mask[n, c, h*std : h*std + self.kernel_size, w*std : w*std + self.kernel_size] = tmp_mask
